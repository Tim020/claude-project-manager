...
- Pull Requests overviews (`AppModel+PullRequests.swift`) use `gh repo view`, `gh pr list/view --json` (`GitHubCLI.overviewFields`) and `gh api graphql` for unresolved review threads. The parsers follow gh's documented JSON; no real output is recorded for them yet. A session belongs to a pull request when its `pullRequestURLs` mention it; sessions whose role is Review count as reviewers.
