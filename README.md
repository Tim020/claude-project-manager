...
- **Files Changed** (design 4a + 4b). Two scopes, switched in either view:
  ...
  - **Which folder:** Files Changed follows the session to where it's actually been working. That's the worktree of its latest edit (Claude may enter a worktree partway through, or a subagent may work in one), else its last working directory within the project. When that differs from the session's folder, both views say "In worktree …".
...
- **Pull Requests** (design 5). The pull requests sessions mention are filled in with live GitHub data through the GitHub CLI (`gh`, which must be installed and signed in). Three scopes:
  - **Project:** the **Pull Requests** row under each project (with how many need attention and how many are open) shows every pull request in the repository in a table: state, checks, review, the sessions that worked on it, lines changed and last update. They're grouped by the folder whose sessions worked on them, then "No Session" for ones started outside Claudio (hide those with **Include PRs without a session**). Filter by Needs Attention (open, with a failing check or changes requested), Open, Merged or All. Click a row for its folder, or on GitHub when no session has it.
  - **Folder:** a folder's PR chip in the sidebar (`#1427 +1`, its dot coloured by what most needs doing) opens a card per pull request: each check with its time, each reviewer's latest review, the sessions on it (click one to open it), and its unresolved review comments.
  - **Session:** the header's PR button (`#1427`) opens a popover with the state, checks, review and unresolved comments of each of the session's pull requests, with View Folder Overview and Open on GitHub.

  The breadcrumbs in a session's header lead to its project's and folder's overviews. Pull requests reload every 2 minutes, or with the refresh button: `gh pr list` for the repository's open ones and its 40 most recent, `gh pr view` for older ones sessions mention, and `gh api graphql` for unresolved review threads.
